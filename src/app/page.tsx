  ...
  return (
    <div className="max-w-3xl mx-auto p-8">
      ...
      <form onSubmit={handleCreate} className="mb-10 flex gap-2">
        <input
          type="text"
          placeholder="Titel der neuen Besprechung..."
      ...
      <div className="space-y-4">
        ...
        {loading ? (
          <p className="text-gray-500 italic">Lade...</p>
        ) : protocols.length === 0 ? (
          ...
          protocols.map((p) => (
            <div key={p.id} className="p-4 bg-white border rounded-lg shadow-sm flex justify-between items-center group hover:border-blue-300 transition-all">
              <div>
                ...
                <span className="text-sm text-gray-500">{new Date(p.date).toLocaleDateString('de-DE')}</span>
              </div>
