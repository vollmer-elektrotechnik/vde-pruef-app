...
import { ClipboardList, Clock, Home, Settings, User, LogOut } from 'lucide-react';
...
export const Sidebar = () => {
  ...
  useEffect(() => {
    ...
    // Höre auf alle Auth-Änderungen (auch das UpdateUser von vorhin!)
  ...
  // Extrahiere die Avatar-URL direkt aus den Live-Sitzungsdaten
  const sidebarAvatarUrl = user?.user_metadata?.avatar_url;

  return (
    <aside className="w-64 bg-slate-900 text-white h-screen fixed left-0 top-0 hidden md:flex flex-col z-50 shadow-2xl">
      <div className="p-6">
        <div className="flex items-center justify-between">
          ...
          >
            {/* Hier wird jetzt das Bild aus den Auth-Metadaten gerendert */}
            {sidebarAvatarUrl ? (
              <img 
            ...
            <div className="min-w-0">
              ...
              <span className="text-[9px] text-slate-500 uppercase font-bold tracking-tight block mt-1">AJV SOLUTIONS</span>
            </div>
          </Link>
      ...
      <nav className="flex-1 px-4 space-y-1 mt-4">
        {menuItems.map((item) => {
          ...
          if (item.href === '/') isActive = pathname === '/';
            ...
            if (item.name === 'Protokolle' && (pathname.startsWith('/protocol/') || pathname.startsWith('/templates'))) {
              isActive = true;
            }
          }
