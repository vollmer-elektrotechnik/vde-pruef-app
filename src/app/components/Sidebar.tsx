...
import { ClipboardList, Clock, Home, Settings, User, LogOut, Menu, X, BookOpen, CheckSquare } from 'lucide-react';
...
export const Sidebar = () => {
  ...
  const menuItems = [
    ...
    { name: 'Anleitungen', icon: <BookOpen size={20} />, href: '/wiki' },
    { name: 'Zeiterfassung', icon: <Clock size={20} />, href: '/time-tracking' },
    { name: 'Aufgaben', icon: <CheckSquare size={20} />, href: '/todos' },
      ...
      `}>
        <div className="p-6">
          <div className="flex items-center justify-between w-full">
            <Link href="/profile" className="flex items-center space-x-3 overflow-hidden group/avatar cursor-pointer select-none">
              {sidebarAvatarUrl ? (
                <img src={sidebarAvatarUrl} alt="Profilbild" className="w-10 h-10 rounded-xl object-cover shadow-lg shadow-blue-500/20 shrink-0 border border-slate-700 transition-transform group-hover/avatar:scale-105" />
              ) : (
            ...
            <div className="flex items-center space-x-1">
              <button onClick={handleLogout} className="p-2 text-slate-500 hover:text-red-400 hover:bg-red-400/10 rounded-lg transition-colors" title="Abmelden">
                <LogOut size={18} />
              </button>
              <button onClick={() => setIsOpen(false)} className="p-2 text-slate-400 hover:text-white rounded-lg md:hidden">
                <X size={20} />
              </button>
            </div>
          </div>
        ...
        <nav className="flex-1 px-4 space-y-1 mt-4">
          {menuItems.map((item) => {
            ...
            if (item.href === '/') isActive = pathname === '/';
              ...
              if (item.name === 'Protokolle' && (pathname.startsWith('/protocols/') || pathname.startsWith('/templates'))) isActive = true;
              if (item.name === 'Anleitungen' && pathname.startsWith('/wiki')) isActive = true;
              if (item.name === 'Aufgaben' && pathname.startsWith('/todos')) isActive = true;
            }

            return (
              <Link key={item.name} href={item.href} className={`flex items-center space-x-3 p-3 rounded-xl transition-all group ${
                isActive ? 'bg-blue-600 text-white shadow-lg shadow-blue-900/20' : 'text-slate-300 hover:bg-slate-800 hover:text-white'
              }`}>
        ...
        <div className="p-6 mt-auto">
          <div className="bg-slate-800/50 rounded-2xl p-4 border border-slate-700/50">
            ...
            <div className="mt-2 text-[10px] text-slate-500">v1.0.3 • Ready for Lexware</div>
          </div>
        </div>
      </aside>
    </>
